...
const PostCard = ({ post, filtered }: PostCardProps) => {
	...
	const sendLikeRequest = async (val: boolean) => {
		likePostMutation.mutate({ postId: post.id, body: { like: val, dislike: !val }, userId: Number(user?.id) });
	};
		...
		<div className={cn(bgColor.current, 'transition flex flex-col gap-4 p-8 rounded-3xl w-full max-w-[700px]')}>
			<div className='post-header flex items-center gap-4'>
				<Avatar className='w-10 h-10'>
					<AvatarImage src={post?.creator?.profileImg} />
				...
				<div className='flex flex-col gap-1 flex-1'>
					...
					<Text className='text-gray-500 text-xs'>{getRelativeTime(post?.updatedAt)}</Text>
				</div>
			...
			<div className='bg-transparent'>
				<Tiptap
					isReadonly
					ref={editorRef}
					content={post?.richDescription ? JSON.parse(post?.richDescription) : null}
